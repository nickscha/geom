...
public class ModelViewProjectionMatrix {
	...
	public void createMVPMatrix() {
		...
		Mat4f modelMatrix = Mat4f.initModelMatrix(myObjectPosInScene, scale);
		...
		Mat4f viewMatrix = Mat4f.initViewMatrix(myObjectPosInScene, rotationMatrix);
		...
		Mat4f projectionMatrix = Mat4f.initProjectionMatrix(90, 1024 / 860, 1, 10);

		// The object to render has been moved from model to view (aka world)
		// space to the projection matrix (rectangle as the screen)
		Mat4f mvpMatrix = Mat4f.initMvpMatrix(modelMatrix, viewMatrix, projectionMatrix);
